...
-----------------
	...
	function lib_shapes.light_toggle(pos, node, clicker)
		node = node or minetest.get_node(pos)
	...
	function lib_shapes.sit_dig(pos, player)
		...
		for _, p in pairs(objs) do
			if not player or not player:is_player() or p:get_player_name() or
					player_api.player_attached[pname] then
				return false
			end
		end
		return true
...
-----------------
	...
	-- Load files
	dofile(lib_shapes.path_mod .. "/beds_api.lua")
	dofile(lib_shapes.path_mod .. "/beds_spawns.lua")
